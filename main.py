...
# b = []
#
# for row in df['К-ть ІПН з майном в іпотеці']:
#     a.append(row)
#
# for row in df['К-ть ІПН з додатковим майном']:
#     b.append(row)
#
...
#         c[key] = 1
#
# df['К-ть ІПН з майном'] = pd.Series(c)
#
# excel_data_df = pd.read_excel('\отчеты\Звіт по іпн.xlsx', sheet_name='Полотно_ІПН')
#
# # print(excel_data_df)
# # print(excel_data_df.columns.ravel()) #список заголовков
#
# listINN = excel_data_df[' К-ть ІПН з платною перевіркою за адресою'].tolist()
#
# df[' К-ть ІПН з платною перевіркою за адресою'] = pd.Series(listINN)
#
...
# plt.show()
#
# df.to_excel('fileTable.xlsx')

# if __name__ == '__main__':
#     df = pd.read_sql_query(input_sql.requestString, input_sql.getConnection())
#     #a = input_sql.addingColumns('К-ть ІПН з майном в іпотеці', 'К-ть ІПН з додатковим майном')
#     propertyAll = input_sql.adding_to_table('К-ть ІПН з майном', input_sql.c)
#     input_sql.adding_to_table(' К-ть ІПН з платною перевіркою за адресою', input_sql.addingfromExcel())
#
#     table = df.groupby('Сегмент боргу').agg({'INN': ['count'],
#                                              'К-ть правильних ІПН': ['sum'],
#                                              'К-ть перевірених ІПН по безкоштовній перевірці': ['sum'],
